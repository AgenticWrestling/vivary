...
func (d *daemon) agentCreate(ctx context.Context, req ctl.AgentCreatePayload) error {
	...
	if templateCfg, err := loadTemplateAgentConfig(subvolPath); err != nil {
		...
		if templateCfg.SchemaErrorRetries != 0 {
			agentCfg.SchemaErrorRetries = templateCfg.SchemaErrorRetries
		}
	}
...
func writeAgentKDL(subvolPath string, cfg AgentConfig) error {
	...
	fmt.Fprintf(&sb, "schema-error-retries %d\n", cfg.SchemaErrorRetries)

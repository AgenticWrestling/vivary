...
Vague verb stems (`Get`, `Do`, `Handle`) and vendor-specific terms (`Gsuite`, `Github`, `Slack`) are rejected by the `vivary-gen` linter at compile time.
...
**4. Compile-time schema generation**

Each capability is implemented as an annotated Go struct. The `vivary-gen` tool processes `jsonschema` struct tags at compile time to generate the static string returned by `Explain()`. This eliminates runtime reflection and ensures the schema the LLM receives is always in sync with what `keeperd`'s binary parses.
...
`vivary-gen` is a `//go:generate` tool that reads Go structs annotated with `jsonschema` tags and emits a companion `_schema.go` file containing the static `Explain()` string. It also runs the capability linter:

- Name matches `Namespace_Noun_Verb` pattern
- All exported fields have a `description` tag
- All exported fields have at least one `example`, `enum`, or `default`
- JSON field names are `snake_case`
- No vendor-specific terms in the namespace or noun segments

```go
// file: email_message_send.go
//go:generate vivary-gen -type=Email_Message_Send

type Email_Message_Send struct {
    DraftID string `json:"draft_id" jsonschema:"description=Draft ID from Email_Message_Create"`
}
```
...
1. Define or update the Go struct in `internal/capabilities/`
2. Run `go generate ./...` to regenerate `_schema.go` files
3. Update `capabilities/capabilities.kdl` with the new/changed entry
4. If the capability operates on a new resource type, add it to `capabilities/entities.kdl` (and any new component schemas to `capabilities/components.kdl`)
5. The `vivary-gen` linter enforces naming and documentation standards — CI fails if any capability violates them

...
This document is a roadmap design note for a post-MVP subsystem. It describes an intended future bridge model, not a currently implemented runtime feature.
...
Bridges act as the "ears" of the system. They would listen for external events and translate them into VIVARY control-plane prompt messages.
...
Bridges act as the "voice" of the system. They would subscribe to event streams from `keeperd` to relay information back to the user.
- The Bridge sends a `MsgType_CtlSubscribe` to `keeperd`.
- `keeperd` pushes `MsgType_CompletionEvent` and `MsgType_FailureEvent` records to the Bridge.
...
## Configuration (proposed future schema)

If bridges are implemented, they will need an explicit config schema and routing policy. The shape below is illustrative rather than currently implemented.
